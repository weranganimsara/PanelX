...
# ---------------------------------------------------------------------------
# Real-Time Network Throughput Monitor (3x-ui style)
# ---------------------------------------------------------------------------
LAST_NET_IO = {"time": 0, "rx": 0, "tx": 0, "rx_rate": 0, "tx_rate": 0}

def get_active_ssh_users() -> dict:
    """
    Returns a dict mapping username -> active SSH connections count.
    Works for both PTY interactive logins and non-PTY / WebSocket tunnel sessions.
    """
    counts = {}
    try:
        res = subprocess.run("ps -eo user,args", shell=True, capture_output=True, text=True)
        for line in res.stdout.splitlines():
            line = line.strip()
            if "sshd:" in line:
                parts = line.split("sshd:")
                if len(parts) > 1:
                    raw_user = parts[1].strip().split()[0].split("@")[0].strip()
                    if raw_user and raw_user not in ["root", "/usr/sbin/sshd"]:
                        uname = raw_user.lower()
...
def get_network_throughput():
    ...
    if dt >= 1.0 and LAST_NET_IO["time"] > 0:
        rx_diff = max(0, total_rx - LAST_NET_IO["rx"])
        tx_diff = max(0, total_tx - LAST_NET_IO["tx"])
        LAST_NET_IO["rx_rate"] = rx_diff / dt
        LAST_NET_IO["tx_rate"] = tx_diff / dt
        LAST_NET_IO["rx"] = total_rx
        LAST_NET_IO["tx"] = total_tx
        LAST_NET_IO["time"] = now
    elif LAST_NET_IO["time"] == 0:
        LAST_NET_IO["rx"] = total_rx
        LAST_NET_IO["tx"] = total_tx
        LAST_NET_IO["time"] = now
    ...
    return {
        "rx_bytes": total_rx,
        "tx_bytes": total_tx,
        "rx_rate_bps": int(LAST_NET_IO["rx_rate"]),
        "tx_rate_bps": int(LAST_NET_IO["tx_rate"]),
        "rx_speed": format_speed(LAST_NET_IO["rx_rate"]),
        "tx_speed": format_speed(LAST_NET_IO["tx_rate"])
    }
...
def os_delete_user(username: str):
    ...
    if not clean_user:
        return
    # 1. Kill any active user processes (sshd, proxies, lingering shells) so userdel doesn't fail
    try:
        subprocess.run(f"pkill -9 -u {clean_user} 2>/dev/null || true", shell=True, timeout=3)
        subprocess.run(f"killall -9 -u {clean_user} 2>/dev/null || true", shell=True, timeout=3)
    except Exception:
        pass

    # 2. Force delete the system user and home directory / mail spool
    try:
        subprocess.run(f"userdel -f -r {clean_user} 2>/dev/null || userdel -r {clean_user} 2>/dev/null || userdel {clean_user} 2>/dev/null || true", shell=True, timeout=5)
    except Exception:
        pass

    # 3. Remove security limits
    try:
        subprocess.run(f"sed -i '/^{clean_user} /d' /etc/security/limits.conf 2>/dev/null || true", shell=True, timeout=3)
    except Exception:
        pass

    # 4. Clean bandwidth usage tracking files
    for base in ["/var/log/panelx/bw", "/etc/panelx/bandwidth"]:
        usage_file = os.path.join(base, f"{clean_user}.usage")
...
class FalconFirewallHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        url = urllib.parse.urlparse(self.path)
        raw_path = url.path
        base_path = get_setting("web_base_path", "/").strip()
        if not base_path:
            base_path = "/"
        ...
        # Transparently map base_path for API or static requests
        path = raw_path
        if base_path != "/" and path.startswith(base_path):
            sub = path[len(base_path)-1:]
            if sub.startswith("/api/") or sub.startswith("/assets/") or sub in ["/favicon.ico", "/favicon.svg"]:
                path = sub
        ...
        # 2. API: System Status & Health
                # API: Real-time Throughput (3x-ui Live Wave)
        elif path == "/api/system/throughput":
            self.send_json(200, get_network_throughput())
            return

        # API: Backup Export (SQLite DB Stream)
            ...
            if os.path.exists(db_path):
                with open(db_path, "rb") as f:
                    db_bytes = f.read()
                filename = f"panelx-backup-{time.strftime('%Y%m%d-%H%M%S')}.db"
                self.send_response(200)
                ...
                return
            self.send_json(404, {"error": "Database file not found"})
            return
        ...
        # 11. API: Users List
            ...
            inb_rows = conn.execute("SELECT id, remark, is_default FROM inbounds").fetchall()
            conn.close()
            users = [dict(r) for r in rows]
            ssh_domain = get_setting("ssh_domain") or get_server_public_ip()
            ssh_port = get_setting("ssh_port", "80")

            active_map = get_active_ssh_users()
            inb_map = {r["id"]: r["remark"] for r in inb_rows}
            default_remark = "Default Gateway"
            for r in inb_rows:
                if r["is_default"]:
                    default_remark = r["remark"]
                    break
            if default_remark == "Default Gateway" and inb_rows:
                default_remark = inb_rows[0]["remark"]

            bw_dir = "/var/log/panelx/bw"
            for u in users:
                u_inb_id = u.get("inbound_id") or 0
                u["inbound_remark"] = inb_map.get(u_inb_id, default_remark)
                uname = u["username"].strip().lower()
                conns = active_map.get(uname, 0)
                u["active_connections"] = conns
                u["is_online"] = conns > 0
        ...
        # 15. Serve Web Static Files / UI
            ...
            if base_path != "/":
                if raw_path == base_path.rstrip("/") or raw_path.startswith(base_path):
                    rel = raw_path[len(base_path)-1:]
                    self.serve_web_ui(rel)
                else:
    ...
    def do_POST(self):
        ...
        # 18. Delete User
            ...
            return
                ...
                # API: Bulk Client Creation (3x-ui Pro Feature)
        elif path == "/api/users/bulk-create":
            prefix = body.get("prefix", "sguser").strip().lower()
            count = min(100, max(1, int(body.get("count", 5))))
            exp_days = int(body.get("exp_days", 30))
            traffic_limit = int(body.get("traffic_limit", 50))
            max_devices = int(body.get("max_devices", 2))
            ...
            for i in range(1, count + 1):
                rand_id = "".join(random.choice("0123456789") for _ in range(3))
                uname = f"{prefix}_{i:02d}_{rand_id}"
                upass = "".join(random.choice(chars) for _ in range(8))
                
                # Create OS user & limits
                os_create_user(uname, upass, exp_date)
                subprocess.run(f"echo '{uname} hard maxlogins {max_devices}' >> /etc/security/limits.conf 2>/dev/null", shell=True)

                conn.execute(
                    """INSERT OR REPLACE INTO users 
                       (username, password, expiry_date, max_devices, traffic_limit_gb, traffic_used_gb, is_locked, created_at) 
                       VALUES (?, ?, ?, ?, ?, 0, 0, ?)""",
                    (uname, upass, exp_date, max_devices, traffic_limit, int(time.time()))
                )
                created_list.append({
                    "username": uname,
                    "password": upass,
            ...
            audit_log("admin", "bulk_user_create", f"Created {count} users with prefix {prefix}", client_ip)
            self.send_json(200, {"success": True, "count": len(created_list), "users": created_list})
            return

        # API: Backup Restore (SQLite Database Import)
        elif path == "/api/backup/restore":
            db_b64 = body.get("database_base64", "")
            if db_b64:
                import base64
                ...
                self.send_json(200, {"success": True, "message": "Database restored successfully"})
                return
            self.send_json(400, {"error": "Missing database payload"})
            return
                # API: Toggle User Lock / Enable (3x-ui Switch)
        elif path == "/api/users/toggle-lock":
            username = body.get("username", "").strip().lower()
            conn = get_db()
            user = conn.execute("SELECT is_locked FROM users WHERE username = ?", (username,)).fetchone()
            if user:
                new_status = 0 if user["is_locked"] else 1
                conn.execute("UPDATE users SET is_locked = ? WHERE username = ?", (new_status, username))
                conn.commit()
                if new_status == 1:
                    subprocess.run(f"usermod -L {username} 2>/dev/null", shell=True)
                    subprocess.run(f"pkill -9 -u {username} 2>/dev/null", shell=True)
                else:
                    subprocess.run(f"usermod -U {username} 2>/dev/null", shell=True)
                conn.close()
                audit_log("admin", "user_toggle_lock", f"User {username} lock state: {new_status}", client_ip)
                self.send_json(200, {"success": True, "is_locked": new_status})
                return
            conn.close()
            self.send_json(404, {"error": "User not found"})
            return

        # API: Edit / Update User (3x-ui Edit Modal)
        elif path == "/api/users/update":
            username = body.get("username", "").strip().lower()
            password = body.get("password", "").strip()
            expiry_date = body.get("expiry_date", "").strip()
            max_devices = int(body.get("max_devices", 2))
            traffic_limit = int(body.get("traffic_limit", 0))
            inbound_id = int(body.get("inbound_id", 0))

            conn = get_db()
            if password:
                conn.execute(
                    "UPDATE users SET password = ?, expiry_date = ?, max_devices = ?, traffic_limit_gb = ?, inbound_id = ? WHERE username = ?",
                    (password, expiry_date, max_devices, traffic_limit, inbound_id, username)
                )
                subprocess.run(f"echo '{username}:{password}' | chpasswd 2>/dev/null", shell=True)
            else:
                conn.execute(
                    "UPDATE users SET expiry_date = ?, max_devices = ?, traffic_limit_gb = ?, inbound_id = ? WHERE username = ?",
                    (expiry_date, max_devices, traffic_limit, inbound_id, username)
                )
            if expiry_date:
                subprocess.run(f"chage -E {expiry_date} {username} 2>/dev/null", shell=True)
            subprocess.run(f"sed -i '/^{username} /d' /etc/security/limits.conf 2>/dev/null", shell=True)
            subprocess.run(f"echo '{username} hard maxlogins {max_devices}' >> /etc/security/limits.conf 2>/dev/null", shell=True)
            conn.commit()
            conn.close()

            audit_log("admin", "user_update", f"User {username} updated", client_ip)
            self.send_json(200, {"success": True, "message": f"User {username} updated successfully"})
            return
            ...
            conn.execute("DELETE FROM sessions WHERE LOWER(username) = LOWER(?)", (username,))
            conn.commit()
            conn.close()
            ...
            try:
                ...
                print(f"[!] Error in os_delete_user({username}): {e}")
    ...
    def serve_web_ui(self, path: str):
        ...
        if os.path.exists(file_path) and os.path.isfile(file_path):
            ...
            if "gzip" in accept_encoding and len(content) > 512:
                ...
                self.send_header("Cache-Control", "no-cache" if file_path.endswith(".html") else "public, max-age=86400")
                self.end_headers()
